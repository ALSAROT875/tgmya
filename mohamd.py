...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        ...
        order = await event.reply('**the سورس 𝒎𝒉𝒎𝒂𝒅 يعمل ⚡️**')
...
@mohamd1.on(events.NewMessage(outgoing=False, pattern='/@mmaahg'))
...
⚝ مرحبا بك في اوامر سورس السيد @mmaahg
...
note : تستخدم هذه الاوامر بأرسالها الى الحساب او بأرسالها الى مجموعة يوجد فيها الحسابات
...
note : ننصحك بوضع عدد الثواني 500
...
`/قائمة التحويل` : الدخول لقائمة تحويل نقاط
...
`/تحويل` : الدخول لقائمة تحويل معلومات
...
`/غادر` : لمغادرة جميع القنوات والمجموعات
...
`/ارسال رساله + الرسالة + يوزر الحساب او البوت`
...
`/نقرات + رقم الزر الشفاف + يوزر البوت`
...
`/اشتراك قناة + يوزر القناة او المجموعة `
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لايوجد قنوات داخل البوت طفي تجميع ورجع بعد ساعه او اقل احسن') != -1:
                ...
                await mohamd1.send_message(event.chat_id, f"تم الانتهاء من التجميع | 𝒎𝒉𝒎𝒂𝒅")
...
@mohamd1.on(events.NewMessage(outgoing=False, pattern='/ممكن تجميع'))
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        await mohamd1.send_message(event.chat_id, "تم الانتهاء من التجميع | 𝒎𝒉𝒎𝒂𝒅")
